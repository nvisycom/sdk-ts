...
import type { Member, UpdateMemberRole, Pagination } from "@/datatypes/index.js";
...
export class MembersService {
  ...
  async get(projectId: string, accountId: string): Promise<Member> {
    const result = await this.#api.GET("/projects/{project_id}/members/{account_id}/", {
   ...
   * @throws {ApiError} if the request fails
   */
  async updateRole(projectId: string, accountId: string, role: UpdateMemberRole): Promise<Member> {
    const result = await this.#api.PATCH("/projects/{project_id}/members/{account_id}/role", {
      params: { path: { projectId, accountId } },
      body: role,
    });
    return unwrap(result);
  ...
  async remove(projectId: string, accountId: string): Promise<void> {
    const result = await this.#api.DELETE("/projects/{project_id}/members/{account_id}/", {
      params: { path: { projectId, accountId } },
    });
    unwrap(result);
  }
  ...
  async leave(projectId: string): Promise<void> {
    const result = await this.#api.POST("/projects/{project_id}/members/leave", {
      params: { path: { projectId } },
    });
    unwrap(result);
  }
