...
import type { Template, CreateTemplate } from "@/datatypes/index.js";
...
export class TemplatesService {
  ...
  async get(projectId: string, templateId: string): Promise<Template> {
    const result = await this.#api.GET("/projects/{project_id}/templates/{template_id}", {
   ...
   * @throws {ApiError} if the request fails
   */
  async update(projectId: string, templateId: string, updates: CreateTemplate): Promise<Template> {
    const result = await this.#api.PUT("/projects/{project_id}/templates/{template_id}", {
      params: { path: { projectId, templateId } },
      body: updates,
    });
    return unwrap(result);
  ...
  async delete(projectId: string, templateId: string): Promise<void> {
    const result = await this.#api.DELETE("/projects/{project_id}/templates/{template_id}", {
      params: { path: { projectId, templateId } },
    });
    unwrap(result);
  }
