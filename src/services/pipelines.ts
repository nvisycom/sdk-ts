...
import type { Pipeline, CreatePipeline } from "@/datatypes/index.js";
...
export class PipelinesService {
  ...
  async get(projectId: string, pipelineId: string): Promise<Pipeline> {
    const result = await this.#api.GET("/projects/{project_id}/pipelines/{pipeline_id}", {
   ...
   * @throws {ApiError} if the request fails
   */
  async update(projectId: string, pipelineId: string, updates: CreatePipeline): Promise<Pipeline> {
    const result = await this.#api.PUT("/projects/{project_id}/pipelines/{pipeline_id}", {
      params: { path: { projectId, pipelineId } },
      body: updates,
    });
    return unwrap(result);
  ...
  async delete(projectId: string, pipelineId: string): Promise<void> {
    const result = await this.#api.DELETE("/projects/{project_id}/pipelines/{pipeline_id}", {
      params: { path: { projectId, pipelineId } },
    });
    unwrap(result);
  }
