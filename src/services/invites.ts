...
export class InvitesService {
  ...
  async cancel(projectId: string, inviteId: string): Promise<void> {
    const result = await this.#api.DELETE("/projects/{project_id}/invites/{invite_id}/", {
   ...
   * @throws {ApiError} if the request fails
   */
  async reply(projectId: string, inviteId: string, reply: ReplyInvite): Promise<Invite> {
    const result = await this.#api.PATCH("/projects/{project_id}/invites/{invite_id}/reply/", {
      params: { path: { projectId, inviteId } },
      body: reply,
    });
    return unwrap(result);
   ...
   * @throws {ApiError} if the request fails
   */
  async generateCode(projectId: string, options: GenerateInviteCode): Promise<InviteCode> {
    const result = await this.#api.POST("/projects/{project_id}/invites/code/", {
      params: { path: { projectId } },
      body: options,
    });
    return unwrap(result);
